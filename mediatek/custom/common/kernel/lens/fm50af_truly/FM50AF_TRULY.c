...
#include "FM50AF_TRULY.h"
#include "../camera/kd_camera_hw.h"

#define LENS_I2C_BUSNUM 1
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO("FM50AF_TRULY", 0x1c)}; // LINE <><20130710><truly len> Jiangde 0x18-->0x1c


#define FM50AF_DRVNAME "FM50AF_TRULY"
...
static int s4FM50AF_ReadReg(unsigned short * a_pu2Result)
{
    ...
    if (i4RetValue < 0) 
    {
        FM50AFDB("[FM50AF TRULY] I2C read failed!! \n");
        return -1;
    }

    // *a_pu2Result = (((u16)pBuff[0]) << 4) + (pBuff[1] >> 4); // Jiangde --   
    *a_pu2Result = (((u16)(pBuff[0] & 0x03)) << 8) + pBuff[1]; // Jiangde ++   
...
static int s4FM50AF_WriteReg(u16 a_u2Data)
{
    ...
    // char puSendCmd[2] = {(char)(a_u2Data >> 4) , (char)(((a_u2Data & 0xF) << 4)+g_sr)}; // Jiangde --    
    char puSendCmd[2] = {(char)(((a_u2Data >> 8) & 0x03) | 0xc4), (char)(a_u2Data & 0xff)}; // Jiangde ++  

    //FM50AFDB("[FM50AF TRULY] g_sr %d, write %d \n", g_sr, a_u2Data);
    g_pstFM50AF_I2Cclient->ext_flag |= I2C_A_FILTER_MSG;
    i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, puSendCmd, 2);
	
    if (i4RetValue < 0) 
    {
        FM50AFDB("[FM50AF TRULY] I2C send failed!! \n");
        return -1;
    }
...
inline static int getFM50AFInfo(__user stFM50AF_TRULY_MotorInfo * pstMotorInfo)
{
    stFM50AF_TRULY_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stFM50AF_TRULY_MotorInfo)))
    {
        FM50AFDB("[FM50AF TRULY] copy to user failed when getting motor information \n");
    }
...
inline static int moveFM50AF(unsigned long a_u4Position)
{
    ...
    {
        FM50AFDB("[FM50AF TRULY] out of range \n");
        return -EINVAL;
    }
    ...
    if (g_s4FM50AF_Opened == 1)
    {
        ...
        if(ret == 0)
        {
            FM50AFDB("[FM50AF TRULY] Init Pos %6d \n", InitPos);
    ...
    //FM50AFDB("[FM50AF TRULY] move [curr] %d [target] %d\n", g_u4CurrPosition, g_u4TargetPosition);
            ...
            {
                FM50AFDB("[FM50AF TRULY] set I2C failed when moving the motor \n");			
                spin_lock(&g_FM50AF_SpinLock);
                g_i4MotorStatus = -1;
                spin_unlock(&g_FM50AF_SpinLock);				
            }
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case FM50AFIOC_G_MOTORINFO :
            i4RetValue = getFM50AFInfo((__user stFM50AF_TRULY_MotorInfo *)(a_u4Param));
        break;
...
static int FM50AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    FM50AFDB("[FM50AF TRULY] FM50AF_Open - Start\n");
    spin_lock(&g_FM50AF_SpinLock);
    if(g_s4FM50AF_Opened)
    {
        spin_unlock(&g_FM50AF_SpinLock);
        FM50AFDB("[FM50AF TRULY] the device is opened \n");
        return -EBUSY;
    }
    ...
    char puSendCmd[2] = {(char)(0xCC) , (char)(0x3B)};
    i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, puSendCmd, 2);

    char puSendCmd2[2] = {(char)(0xD4) , (char)(0x23)}; // 0x23=35
    i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, puSendCmd2, 2);

    char puSendCmd3[2] = {(char)(0xDC) , (char)(0x35)}; // 0x35=53(offset=55-2)
    i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, puSendCmd3, 2);

    char puSendCmd4[2] = {(char)(0xE4) , (char)(0x41)};
    ...
    char puSendCmd2[2] = {(char)(0xD4) , (char)(0x50)};
    i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, puSendCmd2, 2);

    char puSendCmd3[2] = {(char)(0xDC) , (char)(0xB4)};
    i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, puSendCmd3, 2);
    ...
    FM50AFDB("[FM50AF TRULY] FM50AF_Open - End\n");
    return 0;
}
...
{
    FM50AFDB("[FM50AF TRULY] FM50AF_Release - Start\n");

    if (g_s4FM50AF_Opened)
    {
        FM50AFDB("[FM50AF TRULY] feee \n");
        g_sr = 5;
    ...
    FM50AFDB("[FM50AF TRULY] FM50AF_Release - End\n");
...
inline static int Register_FM50AF_CharDrv(void)
{
    ...
    FM50AFDB("[FM50AF TRULY] Register_FM50AF_CharDrv - Start\n");

    //Allocate char driver no.
    if( alloc_chrdev_region(&g_FM50AF_devno, 0, 1,FM50AF_DRVNAME) )
    {
        FM50AFDB("[FM50AF TRULY] Allocate device no failed\n");
    ...
    if(NULL == g_pFM50AF_CharDrv)
    {
        ...
        FM50AFDB("[FM50AF TRULY] Allocate mem for kobject failed\n");
    ...
    {
        FM50AFDB("[FM50AF TRULY] Attatch file operation failed\n");
    ...
    actuator_class = class_create(THIS_MODULE, "actuatordrv_truly"); // LINE <><20130710><truly len> Jiangde, actuatordrv-->actuatordrv_truly
    ...
    FM50AFDB("[FM50AF TRULY] Register_FM50AF_CharDrv - End\n");    
    return 0;
}

inline static void Unregister_FM50AF_CharDrv(void)
{
    FM50AFDB("[FM50AF TRULY] Unregister_FM50AF_CharDrv - Start\n");
    ...
    FM50AFDB("[FM50AF TRULY] Unregister_FM50AF_CharDrv - End\n");    
}
...
struct i2c_driver FM50AF_truly_i2c_driver = {                       
...
static int FM50AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    FM50AFDB("[FM50AF TRULY] FM50AF_i2c_probe\n");

    /* Kirby: add new-style driver { */
    g_pstFM50AF_I2Cclient = client;

    g_pstFM50AF_I2Cclient->addr = FM50AF_VCM_WRITE_ID; // LINE <><20130710><truly len> Jiangde, change to real one
    ...
    if(i4RetValue){
        ...
        FM50AFDB("[FM50AF TRULY] register char device failed!\n");
    ...
    FM50AFDB("[FM50AF TRULY] Attached!! \n");

    return 0;
}

static int FM50AF_truly_probe(struct platform_device *pdev)
{
    return i2c_add_driver(&FM50AF_truly_i2c_driver);
}

static int FM50AF_truly_remove(struct platform_device *pdev)
{
    i2c_del_driver(&FM50AF_truly_i2c_driver);
    return 0;
}

static int FM50AF_truly_suspend(struct platform_device *pdev, pm_message_t mesg)
{
    return 0;
}

static int FM50AF_truly_resume(struct platform_device *pdev)
{
    return 0;
}

// platform structure
static struct platform_driver g_stFM50AF_truly_Driver = { // LINE <><20130710><truly len> Jiangde,
    .probe      = FM50AF_truly_probe,
    .remove     = FM50AF_truly_remove,
    .suspend    = FM50AF_truly_suspend,
    .resume	    = FM50AF_truly_resume,
    .driver     = {
        .name   = "lens_actuator_truly", // LINE <><20130710><truly len>Jiangde
        .owner  = THIS_MODULE,
    }
};

// BEGIN <><20130710><truly len> Jiangde
static struct platform_device actuator_truly_treater = {
	.name   = "lens_actuator_truly",
	.id     = -1,
};
// END <><20130710><truly len> Jiangde


static int __init FM50AF_i2C_init(void)
{
    i2c_register_board_info(LENS_I2C_BUSNUM, &kd_lens_dev, 1);
    platform_device_register(&actuator_truly_treater);// LINE <><20130710><truly len> Jiangde
 
    if(platform_driver_register(&g_stFM50AF_truly_Driver)){
...
{
	platform_driver_unregister(&g_stFM50AF_truly_Driver);
}
