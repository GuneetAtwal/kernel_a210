...
#define MELFAS_I2C_ADDRESS   (0x39)
...
void tpd_hw_enable(void)
{
    ...
    hwPowerOn(MT65XX_POWER_LDO_VGP5, VOL_3000, "TP");//TOUCH  LDO
    msleep(50);
...
void tpd_hw_disable(void)
{
    ...
    msleep(50);
    hwPowerDown(MT65XX_POWER_LDO_VGP5, "TP");//TOUCH  LDO
}
...
static void mms_report_input_data(struct mms_ts_info *info, u8 sz, u8 *buf)
{
	...
	int key_code;
	int key_state;
	int touch_state;//anderson add
	u8 *tmp;

    TPD_DMESG("[melfas_tpd]  mms_report_input_data  %s\n", __func__);
	...
	for (i = 0; i < sz; i += FINGER_EVENT_SZ) {
		...
		if (tmp[0] & MMS_TOUCH_SCREEN_EVENT)  {
			id = (tmp[0] & 0xf) -1;
			x = tmp[2] | ((tmp[1] & 0xf) << 8);
			y = tmp[3] | (((tmp[1] >> 4 ) & 0xf) << 8);
			touch_major = tmp[4];
			pressure = tmp[5];
			touch_state = (tmp[0] & 0x80) ? 1 : 0;//anderson add
			...
			//pr_info("[TSP] %s :Touch ID=: %d, Touch State=: %d, X=: %d, Y=: %d\n",__FUNCTION__,id,touch_state,x,y);//anderson add
		}
		else
		{
			switch (tmp[0] & 0xf) {
			case 1:
				...
				break;
			case 2:
				key_code = KEY_HOMEPAGE;//KEY_BACK;
				break;
                        case 3:
                            key_code = KEY_BACK;
                            break;
			default:
				dev_err(&client->dev, "unknown key type input data\n");
				goto out;
				break;
			}
		  ...
		  #define  MMS_READ_BYTE  8
			for (i = 0 ; i < cnt ;  i += MMS_READ_BYTE){
			  if( i <= cnt - MMS_READ_BYTE){
				...
				//dev_info(&client->dev, "Melfas touch anderson for cm-delta read\n");//anderson
			  }		
			}
			...
			//			for (i = 0; i < read_num; i = i + 6)  // New firware 8byte per point
...
int mms_get_tpd_fw_version(void)
{
    ...
    mms_get_fw_version(melfas_i2c_client,product_id);
    
    return product_id[3];
}


void mms_get_tpd_vendor_name(char * fw_vendor_name)
{
    int r;
    char product_id ;

     //snprintf(fw_vendor_name, PAGE_SIZE, "%d\n", touch_fw_version);
    
    i2c_smbus_read_i2c_block_data(melfas_i2c_client, MELFAS_CHIP_FW_VERSION, 1,&product_id);
	//TPD_DMESG("[melfas_tpd] %s\n", __func__);
...
//LINE<JIRA_ID><DATE20130422><add multi tp>zenghaihui
    .tpd_get_fw_version = mms_get_tpd_fw_version,
    .tpd_get_fw_vendor_name = mms_get_tpd_vendor_name,
};
