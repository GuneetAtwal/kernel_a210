...
static int HalTscrCDevWriteI2CSeq(struct i2c_client *client, u8* data, u16 size)
{
    ...
    {
		{
			...
			.len = size,
			#ifdef __MSG_DMA_MODE__
			.addr = 0x48 & I2C_MASK_FLAG | I2C_DMA_FLAG,
			.buf = g_dma_buff_pa,
			#else
			.addr = client->addr,
			.buf = data,
	...
	if( rc != 1 )
    {
		TPD_DMESG("HalTscrCDevWriteI2CSeq error %d,addr = %d\n", rc, client->addr);
                /*
                rc = i2c_transfer(client->adapter, msgs1, 1);
                if(rc != 1){
                    TPD_DMESG("HalTscrCDevWriteI2CSeq1111 error %d,addr = %d\n", rc, client->addr);
                }*/
	}
...
static int get_fw_version(struct i2c_client *client, u8 *buf)
{
	...
	struct i2c_msg msg[2] = {
        ...
        TPD_DMESG("[get_fw_version] client->addr = 0x%x\n", client->addr);
...
static int mms_isc_transfer_cmd(struct mms_ts_info *info, int cmd)
{
	...
	FUNC_START
		TPD_DMESG("[mms_isc_transfer_cmd] client->addr=%d \n",client->addr);
...
static int mms_flash_section(struct mms_ts_info *info, struct mms_fw_img *img, const u8 *data)
{
	...
	for (page = img->start_page; page <= img->end_page; page++) {
                TPD_DMESG("pchj section [%d] update succeeded\n", page);
...
static int mms_flash_fw(const u8 *fw, struct mms_ts_info *info)
{
	...
	while (retires--) {
		if (!get_fw_version(client, ver)){
            TPD_DMESG("failed to obtain ver1. info\n");
			memcpy(info->ver,ver,MAX_SECTION_NUM); 
			break;
		}else{
		        TPD_DMESG("failed to obtain ver2. info\n");
			mms_reboot(info);
		}
	}
	...
	if (get_fw_version(client, ver)) {
		...
		goto out;
	        ...
	        TPD_DMESG("version mismatch after flash.ver[0]=%d ,ver[1]=%d,ver[2]=%d",ver[img[0]->type,ver[img[1]->type]],ver[img[2]->type]);
	        TPD_DMESG("version mismatch after flash.tar[0]=%d ,tar[1]=%d,tar[2]=%d",target[0],target[1],target[2]);
		for (i = 0; i < fw_hdr->section_num; i++) {
			if (ver[img[i]->type] != target[i]) {
			TPD_DMESG("version mismatch after flash. [%d] 0x%x != 0x%x   version=%d \n",
					i, ver[img[i]->type], target[i],ver[img[i]->type]);
...
void mms_fw_update_controller(const struct firmware *fw, void * context)
{
	...
	*/
        TPD_DMESG("[melfas_tpd]failed to flash firmware after retires11111\n");
